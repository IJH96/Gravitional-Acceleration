...
x,y,z = input("Enter the location for acceleration value:").split()

position = np.array([float(x),float(y),float(z)])


a_x = -centDiffGrav3D(gravPotential, position, h,0,particles)
a_y = -centDiffGrav3D(gravPotential, position,h,1, particles)
a_z = -centDiffGrav3D(gravPotential, position,h,2, particles)
